...
describe("mock gateway", () => {
  ...
  describe("rois.system.get_profile", () => {
    it("returns a canned HRI_Engine_Profile with return_code OK", async () => {
      ...
      // The profile is a JSON string per the RoIS protocol (section 6.2).
  ...
  describe("rois.command.release", () => {
    // PersonDetection_0 is bound by the bind test above, so release it here.
    // The shared registry means bind state persists across test connections.
    it("returns OK for a registered component", async () => {
      const socket = await connect();
      // Bind first to ensure it is in a bound state.
      await roundtrip(
        socket,
        JSON.stringify({
          jsonrpc: "2.0",
  ...
  describe("rois.command.get_parameter", () => {
    it("returns all parameters when names is omitted", async () => {
      ...
      expect(reply.result.return_code).toBe("OK");
      expect(reply.result.parameters).toHaveLength(3);
      const names = reply.result.parameters.map((r: any) => r.name);
    ...
    it("returns only requested parameters when names is provided", async () => {
      ...
      expect(reply.result.return_code).toBe("OK");
      expect(reply.result.parameters).toHaveLength(1);
      expect(reply.result.parameters[0].name).toBe("time_limit");
      expect(reply.result.parameters[0].value).toBe("30");
      socket.close();
    });
  ...
  describe("rois.command.set_parameter", () => {
    ...
    it("persists values that get_parameter can read back", async () => {
      ...
      expect(reply.result.return_code).toBe("OK");
      expect(reply.result.parameters).toHaveLength(1);
      expect(reply.result.parameters[0].value).toBe("120");
      socket.close();
    });
