...
/**
 * RemoteSubEngine: one WebSocket connection to one adapter process.
 *
 * Implements SubEngine by forwarding JSON-RPC requests over the WebSocket
 * and awaiting matching responses. Event notifications from the adapter
 * are routed to the EventSink associated with the subscribe_id.
 *
 * Multiple RemoteSubEngine instances can exist simultaneously, one per
 * connected adapter.
 */
export class RemoteSubEngine implements SubEngine {
  ...
  handleRegister(msg: Record<string, unknown>): void {
    ...
    this.components = rawComponents.map((c) => ({
      ref: String(c.ref ?? ''),
      type: String(c.type ?? ''),
      bindRequired: Boolean(c.bind_required ?? false),
      queries: (c.queries as string[]) ?? [],
      commands: (c.commands as string[]) ?? [],
      events: (c.events as string[]) ?? [],
    }));
    console.log(
      `[sub-engine] registered "${this.engineId}" with ${this.components.length} components`,
    );
    // Respond with OK.
